...
- [ ] **Unused EntryRules fields**: `strategy/spec.py` defines 12 fields in `EntryRules`, but only 2 are consumed in the live execution path: `obi_threshold` (by `RuleBasedScorer`) and `spread_max_bps` (by `gate_4_order_selection()`). The remaining 10 fields exist in YAML strategy specs but are silently ignored — the live code reads from `settings.*` directly: `micro_price_move_floor_bps` and `micro_price_move_percentile` (microstructure.py reads `settings.MICRO_PRICE_MOVE_FLOOR_BPS` / `settings.MICRO_PRICE_MOVE_PERCENTILE` via `rolling_abs_move_threshold()`), `protection_max_distance_bps` (executor.py line 476 reads `settings.PROTECTION_MAX_DISTANCE_BPS`), `micro_max_hold_ms` (PersistenceMonitor reads `settings.MICRO_MAX_HOLD_MS`), `micro_exit_spread_hard_cap_bps` (PersistenceMonitor reads `settings.MICRO_EXIT_SPREAD_HARD_CAP_BPS`), `atr_mult_sl` and `atr_mult_tp` (wall-based SL is used in live path; `OrderManager` reads `settings.ATR_MULTIPLIER_TP` directly), plus `cvd_momentum_min`, `vol_ratio_min`, and `sweep_qty_mult` (no consumer anywhere). Per-strategy override of these thresholds via YAML is therefore inert. Either inject the relevant `entry_rules` fields into the consuming components (microstructure.py, executor.py, order_manager.py) or remove the dead fields and document that `settings.*` are the authoritative knobs.
