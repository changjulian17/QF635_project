...
try:
    ...
    with left:
        ...
        if not ms.empty:
            ...
            combined = ask_matrix - bid_matrix
            ...
            if buy_mask.any():
                fig_hm.add_trace(go.Scatter(
                    x=hm_df.loc[buy_mask, "ts"], y=hm_df.loc[buy_mask, "mid_price"],
                    mode="markers",
                    marker=dict(size=np.clip(hm_df.loc[buy_mask, "buy_volume"] * 40, 4, 24),
                                color="rgba(0,255,100,0.6)", line=dict(width=0)),
                    name="Buy aggression",
            ...
            if sell_mask.any():
                fig_hm.add_trace(go.Scatter(
                    x=hm_df.loc[sell_mask, "ts"], y=hm_df.loc[sell_mask, "mid_price"],
                    mode="markers",
                    marker=dict(size=np.clip(hm_df.loc[sell_mask, "sell_volume"] * 40, 4, 24),
                                color="rgba(255,60,60,0.6)", line=dict(width=0)),
    ...
    with right:
        if not ms.empty:
            ...
            st.plotly_chart(fig_sub, use_container_width=True, key="obi_cvd")
